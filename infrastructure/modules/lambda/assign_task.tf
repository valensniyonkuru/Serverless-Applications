    ...
    function_name = "task-management-assign-task"
    handler = "assign_task.handler"
    runtime = "nodejs20.x"
    role = var.lambda_role_arn
    filename = "${path.root}/../backend/dist/assign_task.zip"
    source_code_hash = filebase64sha256("${path.root}/../backend/dist/assign_task.zip")
    memory_size = 128
    timeout = 10
  ...
  resource "aws_apigatewayv2_integration" "assign_task_integration" {
    api_id = var.api_gateway_id
    integration_type = "AWS_PROXY"
    integration_uri = aws_lambda_function.assign_task.arn
